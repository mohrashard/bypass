...
# 1. Update pipeline orchestration order
# Find the main pipeline block
start_marker = "# ─── MAIN PIPELINE SEQUENCE ───────────────────────────────────────────────"
if start_marker in code:
    # Remove STEP 3: Remove Dead Air from its current spot
    step3_pattern = r"    # STEP 3: Remove Dead Air / Jump Cuts\n    if options\.get\(\"removeSilence\"\):\n        current_video = stage_remove_silence\(current_video, options\)\n"
    code = re.sub(step3_pattern, "", code)
    
    # Insert it after STEP 4 (which is now background fx)
    step4_pattern = r"(    # STEP 4: Background FX \+ Sandwich Text.*?current_video = stage_background_fx\(current_video, options\)\n)"
    replacement = r"\1\n    # STEP 4.5: Remove Dead Air / Jump Cuts (MOVED AFTER BG FX)\n    if options.get(\"removeSilence\"):\n        current_video = stage_remove_silence(current_video, options)\n"
    code = re.sub(step4_pattern, replacement, code, flags=re.DOTALL)

# 2. Rewrite stage_scene_transitions
...
old_transitions_pattern = r"def stage_scene_transitions\(video_path: str, options: dict\) -> str:.*?return video_path"
code = re.sub(old_transitions_pattern, new_transitions, code, flags=re.DOTALL)
